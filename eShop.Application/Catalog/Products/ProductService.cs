...
namespace eShop.Application.Catalog.Products
{
    public class ProductService : IProductService
    {
        ...
        public async Task<PagedResult<ProductViewModal>> GetAllPaging(GetManageProductPagingRequest request)
        {
            //1. Select join
            ...
            var pagedResult = new PagedResult<ProductViewModal>()
            {
        ...
        public async Task<ProductViewModal> GetById(int id, string languageId)
        {
            var product = await _context.Products.FindAsync(id);
        ...
        public async Task<ProductImageViewModal> GetImageById(int imageId)
        {
            var image = await _context.ProductImages.FindAsync(imageId);
            if (image == null)
                throw new EShopException($"Cannot find image with id {imageId}");

            var viewModel = new ProductImageViewModal()
               {
        ...
        public async Task<List<ProductImageViewModal>> GetListImages(int productId)
        {
            return await _context.ProductImages.Where(x => x.ProductId == productId)
                .Select(i => new ProductImageViewModal() 
            { 
                Caption = i.Caption,
        ...
        public async Task<PagedResult<ProductViewModal>> GetAllByCategoryId(string languageId, GetPublicProductPagingRequest request)
        {
            //1. Select join
            var query = from p in _context.Products
                        join pt in _context.ProductTranslations on p.Id equals pt.ProductId
